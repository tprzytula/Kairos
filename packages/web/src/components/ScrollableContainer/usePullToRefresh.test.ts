...
describe('usePullToRefresh', () => {
  ...
  it('calls onRefresh and sets isRefreshing when pullDistance >= threshold on release', async () => {
  ...
  it('resets state after onRefresh resolves', async () => {
    const onRefresh = vi.fn().mockResolvedValue(undefined)
    const { result } = renderHook(() => usePullToRefresh(containerRef as any, onRefresh))

    fire('touchstart', createTouchEvent(0))
    fire('touchmove', createTouchEvent(PULL_THRESHOLD))
    await act(async () => { fire('touchend') })

    expect(result.current.isRefreshing).toBe(false)
    expect(result.current.pullDistance).toBe(0)
  })

  it('resets state after onRefresh rejects (no stuck loading state)', async () => {
