...
describe('Given the get_grocery_items_default_units lambda handler', () => {
    it('should make a scan request to the grocery items default units table', async () => {
    ...
    it('should log the response', async () => {
        ...
        expect(logSpy).toHaveBeenCalledWith('Returning items', {
            count: 2,
            items: JSON.stringify(EXAMPLE_DB_GROCERY_ITEMS_DEFAULT_UNITS),
        });
    });
    ...
    it('should return status 200 and a list of grocery items icons', async () => {
        ...
        expect(result.body).toBe(JSON.stringify(EXAMPLE_DB_GROCERY_ITEMS_DEFAULT_UNITS));
    });
...
const mockScan = () => jest.spyOn(DynamoDB, 'scan').mockResolvedValue(EXAMPLE_DB_GROCERY_ITEMS_DEFAULT_UNITS);

const EXAMPLE_DB_GROCERY_ITEMS_DEFAULT_UNITS: Record<string, unknown>[] = [
    {
        name: "Apple",
        unit: "unit(s)",
    },
